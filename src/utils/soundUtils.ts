
// Utility functions for sound effects in the game

// Create click sound using the Web Audio API
export const createClickSound = (): HTMLAudioElement => {
  // Create an in-memory audio element
  const audio = new Audio();
  audio.volume = 0.5;
  
  try {
    // Try to use the Audio Context API to generate a click sound
    const AudioContext = window.AudioContext || (window as any).webkitAudioContext;
    
    if (AudioContext) {
      const audioCtx = new AudioContext();
      const oscillator = audioCtx.createOscillator();
      const gainNode = audioCtx.createGain();
      
      oscillator.type = 'sine';
      oscillator.frequency.setValueAtTime(800, audioCtx.currentTime);
      oscillator.connect(gainNode);
      gainNode.connect(audioCtx.destination);
      
      gainNode.gain.setValueAtTime(0, audioCtx.currentTime);
      gainNode.gain.linearRampToValueAtTime(0.5, audioCtx.currentTime + 0.01);
      gainNode.gain.linearRampToValueAtTime(0, audioCtx.currentTime + 0.1);
      
      oscillator.start();
      oscillator.stop(audioCtx.currentTime + 0.1);
      
      // Create a click sound as a data URI
      const clickSound = `data:audio/wav;base64,`;
      audio.src = clickSound;
      return audio;
    }
  } catch (e) {
    console.log("Web Audio API not supported, falling back to default sound");
  }
  
  // Fallback to a very simple click sound as data URI
  audio.src = "data:audio/mp3;base64,";
  
  return audio;
};

// Function to create and play winner announcement sound
export const playWinSound = (player: 'trump' | 'modi'): void => {
  // Use the Web Speech API to announce the winner
  try {
    const speech = new SpeechSynthesisUtterance();
    speech.volume = 1; // 0 to 1
    speech.rate = 0.9; // 0.1 to 10
    speech.pitch = 1; // 0 to 2
    speech.lang = 'en-US';
    
    if (player === 'trump') {
      speech.text = "Trump has won the match";
    } else {
      speech.text = "Modi has won the match";
    }
    
    window.speechSynthesis.speak(speech);
  } catch (err) {
    console.error("Speech synthesis not supported:", err);
  }
};

// Function to create and play a bell sound
export const playBellSound = (): void => {
  try {
    const AudioContext = window.AudioContext || (window as any).webkitAudioContext;
    
    if (AudioContext) {
      const audioCtx = new AudioContext();
      const oscillator = audioCtx.createOscillator();
      const gainNode = audioCtx.createGain();
      
      oscillator.type = 'sine';
      oscillator.frequency.setValueAtTime(880, audioCtx.currentTime); // A5 note
      oscillator.connect(gainNode);
      gainNode.connect(audioCtx.destination);
      
      // Bell-like envelope
      gainNode.gain.setValueAtTime(0, audioCtx.currentTime);
      gainNode.gain.linearRampToValueAtTime(0.8, audioCtx.currentTime + 0.01);
      gainNode.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + 1.5);
      
      oscillator.start();
      oscillator.stop(audioCtx.currentTime + 1.5);
    }
  } catch (err) {
    console.error("Web Audio API not supported for bell sound:", err);
  }
};
